...
def seedmachine_single(pathinfo, sites, pathyaml, basic, qbinfo, imgdata, hashlist):
    ...
    for pathep in pathinfo.eps:
        ...
        for siteitem in sites:
            # 发布过此集的站点略过
            if sites[siteitem].enable == 1 and not (eval('pathinfo.' + siteitem + '_max_done==10000')) and not eval(
                    'pathep in pathinfo.' + siteitem + '_done'):
        ...
        if file1.complete == 1 and (
                not 'complete' in pathyaml or pathyaml['complete'] == None or pathyaml['complete'] == ''):
            pathyaml['complete'] = 1
        ...
        for siteitem in site_upload:
            if siteitem.enable == 0:
                continue
            logger.info(
                '正在' + siteitem.sitename + '站点发布路径' + pathinfo.chinesename + '第' + str(pathep) + '集资源')
            ...
            if not upload_success:
                ...
                exec(
                    'pathyaml["' + siteitem.sitename + '"]=",".join([str(i) for i in pathinfo.' + siteitem.sitename + '_done])')
            else:
                succnum = succnum + 1
...
def seedmachine_rest(pathinfo, sites, pathyaml, basic, qbinfo, imgdata, hashlist):
    ...
    for siteitem in sites:
        ...
        if pathinfo.zeroday_name != '' and pathinfo.zeroday_name != None and os.path.exists(
                os.path.join(path_old, pathinfo.zeroday_name)):
...
def seedmachine(pathinfo, sites, pathyaml, basic, qbinfo, imgdata, hashlist):
        ...
        if not upload_success:
            ...
            if not 'movie' in pathinfo.type.lower():
                for epitem in pathinfo.eps:
                    exec('pathinfo.' + siteitem.sitename + '_done.append(' + str(epitem) + ')')
                exec(
                    'pathinfo.' + siteitem.sitename + '_done=list(set(' + 'pathinfo.' + siteitem.sitename + '_done' + '))')
                exec('pathinfo.' + siteitem.sitename + '_done.sort()')
    ...
    logger.info('路径' + pathinfo.path + '下资源已全部发布完毕')
    if errorsite == "":
        pushtext = pathinfo.chinesename + "发送完成，发送成功站点数" + str(succnum) + "个,无失败站点"
    else:
        pushtext = pathinfo.chinesename + "发送完成，发送成功站点数" + str(succnum) + "个,失败站点" + str(
            errornum) + "个:" + errorsite
    pushurl = "https://sc.ftqq.com/" + basic['serverchan_api'] + ".send?title=发种成功&desp=" + pushtext
    pushres = requests.get(pushurl)
    return log_error, log_succ


def startupload_machine(pathlist, sites, yamlinfo):
    log_allsucc = ''
    log_allerror = ''
    hashlist = []
