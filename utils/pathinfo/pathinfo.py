...
class pathinfo(object):
    def updatepath(self,newpath):
        ...
        if ('anime' in self.type.lower() or 'tv' in self.type.lower() or 'doc' in self.type.lower()):
    ...
    def __init__(self,pathid,infodict,sites,basic):
        ...
        #可有可无的属性,后面不写入配置文件
        attr_disp=['upload_name','video_type','video_format','audio_format','year','zeroday_name','exinfo','seasonnum','imdb_url','bgm_url','anidb_url','transfer','txt_info','audio_info','from_url','contenttail','contenthead','screenshot','small_descr','source','tags','medium','tmdb_id','imdb_id','tmdb_name',"filename"]
        for item in attr_disp:
            if not item in infodict or infodict[item]==None:
                exec('self.'+item+'=""')
                exec('self.exist_'+item+'=False')
        ...
        if (not self.exist_doubanurl):
            if ('anime' in self.type or 'tv' in self.type or 'doc' in self.type):
                if self.seasonnum>1:
    ...
    def print(self):
        attr=['path','type','collection','enable','doubanurl','imdb_url','bgm_url','anidb_url','from_url','transfer','sub','englishname','chinesename','tmdb_id','imdb_id','tmdb_name']
        print('Path info:')
        for item in attr:
        ...
        if (self.type=='anime' or self.type=='tv' or 'doc' in self.type.lower):
