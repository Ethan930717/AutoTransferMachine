...
import json
import sys
from utils.edittorrent.edittorrent import *
...
def mktorrent_win_temp(filepath, torrentname, tracker="https://announce.leaguehd.com/announce.php"):
    ...
    order = 'mktorrent -v -p -f -l 24 -a ' + tracker + ' -o \"' + changename(torrentname) + '\" \"' + os.path.basename(
        filepath) + '\"' + ''
    logger.info(order)
    os.system(order)
...
def mktorrent_win(filepath, torrentname, tracker="https://announce.leaguehd.com/announce.php"):
    ...
    if os.path.isdir(filepath):
        ...
        logger.info('正在制作种子:' + filepath)
        order = 'mktorrent -v -p -l 24 -c "Made by Auto_Upload" -a ' + tracker + ' -o \"' + changename(
            torrentname) + '\" \"' + os.path.basename(new_filepath) + '\"' + ''
        # logger.info(order)
...
def mktorrent(filepath: str, torrentname: str, tracker="https://announce.leaguehd.com/announce.php"):
    ...
    while filesize == 0:
        ...
        if tracker:
            torrent = Torrent(path=str(path.absolute()), trackers=[tracker], piece_size=piece_size, private=private,
...
class mediafile(object):
    def __init__(self, mediapath, pathinfo, basic, imgdata):
        ...
        if os.path.isdir(self.mediapath):
            ...
            if self.pathinfo.collection >= 1 and 'zeroday_name' in self.pathinfo.infodict and self.pathinfo.infodict[
                'zeroday_name'] != None and self.pathinfo.infodict['zeroday_name'] != '':
                ...
                if os.path.exists(zeroday_path):
                    ...
                    for i in ls:
                        ...
                        if (os.path.isdir(c_path)) or (i.startswith('.')) or (not (
                                os.path.splitext(i)[1].lower() == ('.mp4') or os.path.splitext(i)[1].lower() == (
                                '.mkv') or os.path.splitext(i)[1].lower() == ('.avi') or os.path.splitext(i)[
                                    1].lower() == (
                                        '.ts'))):
                            continue
                        filesize = os.path.getsize(c_path)
    ...
    def getaudio(self):
        ...
        for item in infolist:
            ...
            if item['@type'].lower() == 'audio':
                if 'Title' in item:
                    if 'CHINESE' in item['Title'].upper() or '中' in item['Title'].upper() or 'CH' in item[
                        'Title'].upper() or 'ZH' in item['Title'].upper() or '国' in item[
                        'Title'].upper() or 'MANDARIN' in item['Title'].upper() or '国语' in self.pathinfo.small_descr:
                        ch = 1
                    ...
                    if 'EN' in item['Title'].upper() or '英' in item['Title'].upper():
                        en = 1
                    if 'CANTON' in item['Title'].upper() or '粤' in item[
                        'Title'].upper() or '粤语' in self.pathinfo.small_descr:
                        yue = 1
                elif 'Language' in item:
                    if 'CHINESE' in item['Language'].upper() or '中' in item['Language'].upper() or 'CH' in item[
                        'Language'].upper() or 'ZH' in item['Language'].upper() or '国' in item[
                        'Language'].upper() or '国语' in self.pathinfo.small_descr:
                        ch = 1
                    if 'JA' in item['Language'].upper() or 'JP' in item['Language'].upper() or '日' in item[
                        'Language'].upper():
                        jp = 1
                    if 'EN' in item['Language'].upper() or '英' in item['Language'].upper():
                        en = 1
                    if 'CANTON' in item['Language'].upper() or '粤' in item[
                        'Language'].upper() or '粤语' in self.pathinfo.small_descr:
                        yue = 1
        '''
        if jp==0 and ch==0 and en==1:
            self.language='英语'
    ...
    def getsubtext(self):
        ...
        for item in infolist:
            ...
            if item['@type'].lower() == 'text':
                if 'Title' in item:
                    if 'TC' in item['Title'].upper() or 'CHT' in item['Title'].upper() or 'BIG5' in item[
                        'Title'].upper() or '繁' in item['Title'].upper():
                        tc = 1
                    elif 'SC' in item['Title'].upper() or 'CHS' in item['Title'].upper() or 'GB' in item[
                        'Title'].upper() or '简' in item['Title'].upper() or '簡' in item['Title'].upper() or '中' in \
                            item['Title'].upper():
                        sc = 1
                    elif 'JP' in item['Title'].upper() or 'JAPANESE' in item['Title'].upper() or '日' in item[
                        'Title'].upper():
                        jp = 1
                    elif 'EN' in item['Title'].upper() or 'ENGLISH' in item['Title'].upper() or '英' in item[
                        'Title'].upper():
                        en = 1
                elif 'Language' in item:
                    if 'TC' in item['Language'].upper() or 'CHT' in item['Language'].upper() or 'BIG5' in item[
                        'Language'].upper() or '繁' in item['Language'].upper():
                        tc = 1
                    elif 'SC' in item['Language'].upper() or 'CHS' in item['Language'].upper() or 'GB' in item[
                        'Language'].upper() or '简' in item['Language'].upper() or '簡' in item[
                        'Language'].upper() or 'ZH' in item['Language'].upper():
                        sc = 1
                    elif 'JP' in item['Language'].upper() or 'JA' in item['Language'].upper() or 'JAPANESE' in item[
                        'Language'].upper() or '日' in item['Language'].upper():
                        jp = 1
                    elif 'EN' in item['Language'].upper() or 'ENGLISH' in item['Language'].upper() or '英' in item[
                        'Language'].upper():
                        en = 1
        if en == 0 and jp == 0 and sc == 0 and tc == 1:
            self.sublan = '[繁体中字]'
    ...
    def getfullinfo(self, tracker='https://announce.leaguehd.com/announce.php'):
        ...
        if self.pathinfo.type == 'anime' or self.pathinfo.type == 'tv' or self.pathinfo.type == 'doc':
            ...
            if not self.isdir:
                ...
                eps_temp.sort()
                if len(eps_temp) > 1:
                    self.uploadname = self.uploadname + 'E' + str(eps_temp[0]).zfill(2) + '-E' + str(
                        eps_temp[-1]).zfill(2)
                    self.small_descr = self.small_descr + ' | 第' + str(eps_temp[0]).zfill(2) + '-' + str(
                        eps_temp[-1]).zfill(2) + '集'
                else:
                    self.uploadname = self.uploadname + 'E' + self.episodename
                    self.small_descr = self.small_descr + ' | 第' + self.episodename + '集'
                ...
                self.uploadname = self.uploadname + 'E' + str(self.pathinfo.min).zfill(2) + '-E' + str(
                    self.pathinfo.max).zfill(2)
