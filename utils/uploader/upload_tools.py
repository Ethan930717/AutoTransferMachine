...
def afterupload(r, fileinfo, record_path, siteinfo, file1, qbinfo, hashlist):
    ...
    if r.status_code == 200:
        logger.info('已发布成功')
    elif r.status_code == 400 and siteinfo.sitename == 'zhuque' and 'code' in r.json() and 'TORRENT_ALREADY_UPLOAD' in \
            r.json()['code']:
        return True, fileinfo + '种子发布失败,失败原因:种子已存在'
    elif siteinfo.sitename == 'zhuque' and r.status_code == 400 and 'code' in r.json():
        logger.warning(
            '发布种子发生错误，错误代码:' + str(r.status_code) + ' ,错误信息:' + str(r.reason) + '站点返回信息:' +
            r.json()['code'])
    ...
    if siteinfo.sitename == 'zhuque':
        ...
        parsed_url = urlparse(downloadurl)  # 解析 URL
        query = parsed_url.query  # 获取 query 参数
        params = parse_qs(query)  # 解析 query 参数
        try:
            torrent_id = params["torrent_id"][0]  # 获取 torrent_id 的值
        except KeyError:  # 捕获 KeyError 异常
            torrent_id = "0"  # 使用一个默认值
        downloadurl = f"https://www.haidan.video/download.php?id={torrent_id}&passkey={siteinfo.passkey}"
    else:
...
def qbseed(url, filepath, qbinfo, is_skip_checking=True, is_paused=False, category=None, hashlist=[]):
    ...
    while tor_num_new == tor_num:
        ...
        try:
            res = client.torrents_add(urls=url, save_path=filepath, is_skip_checking=is_skip_checking,
                                      is_paused=is_paused, use_auto_torrent_management=None, category=category)
