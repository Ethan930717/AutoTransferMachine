...
class mediafile(object):
            ...
            if response.status_code == 200:
                ...
                if data["results"]:
                    ...
                    if not tmdbname:
                        ...
                        tmdbid = ""
                    # 输出TMDB ID
                    print(f"The TMDB ID of the first result for '{tmdbname}' is {tmdbid}.")
    ...
    def getfullinfo(self,tracker='https://announce.leaguehd.com/announce.php'):
        ...
        if self.pathinfo.complete == 1:
            self.uploadname = self.uploadname+' Complete '+self.standard_sel+' '+self.type+' '+self.Video_Format+' '+self.Audio_Format+(self.audio_num>1)*(' '+str(self.audio_num)+'Audio') +'-'+self.sub
        else:
